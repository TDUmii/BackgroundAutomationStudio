...
Pointer-path recording is a native semantic checkbox below foreground timing. It is off by default, names Engine 2 and physical pointer ownership in helper text, and remains keyboard operable. The Studio recorder samples visible movement rather than storing every Windows move message, keeps every point client-relative, and shows each captured point as one editable Move pointer action with its delay attached. While the left button is held, Studio suspends path sampling so the existing gesture remains one editable Drag action instead of becoming a trail of Move actions. Foreground Recorder Mini uses the same option and terminology in its target card, but its deliberately smaller step model records movement and clicks without converting drag gestures into held-button Drag actions. During recording, both variants ignore movement outside the selected target or while another window has focus.
